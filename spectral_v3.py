...
This version incorporates the theoretical guarantees from the
*finite‑length closure* note (Part (i)) by running the greedy
proximity–redundancy walk **in the projected space**
   \widehat x := P(x−μ)/‖P(x−μ)‖.  All angular computations therefore use
the hollow‑norm projector that the user selects via *mode*.

Key changes
-----------
• **Projected chain vectors** – a single helper builds L2‑normalised
  projected embeddings once at start‑up; all /chain queries now work in
  that space, guaranteeing finite length by design.
• Updated *console* & *chain* glue code to use the projected vectors.
• No further signature changes for the public functions or CLI flags; all
  scripts and notebooks that relied on previous behaviour continue to
  work, but the chain results are now mathematically aligned with the
  theory.
"""
...
def cheb_poly_coeffs(f: Callable[[np.ndarray], np.ndarray],
    ...
    """Return power‑basis coefficients *c_k* that minimise
       ‖f(λ) − Σ_k c_k λ^k‖_∞ on [0, λ₁]."""
...
def polynomial_matrix(A: np.ndarray,
    ...
    """Compute  P = Σ_k c_k A^k  via Horner; O(deg·d³)."""
    d = A.shape[0]
...
def smooth_projector(λ: np.ndarray, U: np.ndarray,
    ...
    """Find θ* s.t. removed‑variance fraction = ρ."""
    lo = 1e-12
    hi = λ.max()
...
def poly_projector(Σ: np.ndarray,
    ...
    """Return P ≈ w(Σ) and the sparse coefficient vector."""
    if mask == "smooth":
        if theta is None:
    ...
    print(f"[INFO] poly-{mask}: degree={deg}, kept {nonzero}/{deg+1} monomials (ε={eps:g})")
    P_mat = polynomial_matrix(Σ, coeffs)
    return P_mat, coeffs


# ──────────────────────────────────────────────────────────────────────────────
# Helper  – Projected-and‑normalised chain vectors
# ──────────────────────────────────────────────────────────────────────────────

def build_chain_vectors(raw_vectors: np.ndarray,
                        mu: np.ndarray,
                        transform: np.ndarray | Callable[[np.ndarray], np.ndarray]) -> np.ndarray:
    """Return ‖P(x−μ)‖‑normalised vectors for every token."""
    if callable(transform):
        Y = np.stack([transform(v) - mu for v in raw_vectors])
    else:  # matrix
        Y = (raw_vectors - mu) @ transform
...
# ──────────────────────────────────────────────────────────────────────────────
# 5.  Semantic‑chain algorithm (unchanged internally)
# ──────────────────────────────────────────────────────────────────────────────
...
def console(kv: KeyedVectors,
            ...
            chain_vectors: np.ndarray) -> None:
    """Interactive REPL.  The /chain command now uses *chain_vectors* (projected)."""
    ...
    while True:
        ...
        if q.startswith("/chain"):
            ...
            chain = generate_sentence_chain(tok_seq, kv, chain_vectors, λ_, τ_)
            if chain:
                print("\n" + "  ".join(chain) + "\n")
            else:
        ...
        if mode == "whiten":
            vec_t = transform(vec)               # still unused in REPL
...
def main(argv: List[str] | None = None) -> None:
    ...
    raw_vectors = kv.vectors

    mu = raw_vectors.mean(axis=0)
    E_hat = raw_vectors - mu
    ...
    if mode == "smooth":
        ...
        h = mahala_norms(raw_vectors, mu, Σ_isqrt)
    ...
    # ─── build projected chain vectors once (finite‑length guarantee) ─────────
    chain_vectors = build_chain_vectors(raw_vectors, mu, transform)

    console(kv, mu, transform, h, args.eps, mode, locals().get('Σ_isqrt'), chain_vectors)
